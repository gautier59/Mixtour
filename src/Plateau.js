...
var Plateau = function () {
    ...
    this.generateGameIA = function () {
        ...
        if (cellTowerSame.length > 0) {
            ...
            /// R�cup�ration de notre TOWER basse
    ...
    this.getTowerWithMorePionParameter = function (heightmin, lineTowerHigher, columnTowerHigher) { // R�cup�re la liste des cells avec des colonnes qui ont au minimum la hauteur passer en param�tre et diff�rent de la cell de la tour haute
        ...
        for (var i = 0; i < 5; i++) {
            for (var j = 0; j < 5; j++) {
                if (this.getListPionPosition(i, j).length >= heightmin) { // On est sur une cells qui contient une tour
                    if ((i != lineTowerHigher) || (j != columnTowerHigher)) { // V�rifier si on est pas sur la m�me cellules que la tour haute !
                        var pionCell = this.getListPionPosition(i, j);
